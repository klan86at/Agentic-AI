            ...
            <div className="space-y-2">
              {message.split('```').map((part, index) => {
                if (index % 2 === 1) {
                  ...
                  return (
                    <div key={index} className="bg-chat-code rounded-xl p-3 border border-border/30 hover:border-primary/20 transition-colors">
                ...
                return <div key={index} className="whitespace-pre-wrap leading-relaxed text-sm">{part}</div>;
              })}
            </div>
          ) : (
