...
class Tool:
    ...
    def __post_init__(self) -> None:
        ...
        # Store the underlying function for introspection
        self._func_obj = self.func.__func__ if hasattr(self.func, "__func__") else self.func
        self._type_hints = get_type_hints(self._func_obj)
        self.description = Tool.get_func_description(self._func_obj)
        if hasattr(self._func_obj, "_jac_semstr_inner"):
            self.params_desc = self._func_obj._jac_semstr_inner  # type: ignore
        else:
            self.params_desc = {
                name: str(type_) for name, type_ in self._type_hints.items()
            }
    ...
    def get_json_schema(self) -> dict[str, object]:
        ...
        return tool_to_schema(self._func_obj, self.description, self.params_desc)

    def parse_arguments(self, args_json: dict) -> dict:
        """Parse the arguments from JSON to the function's expected format."""
        args = {}
        # Use the cached type hints from __post_init__
        annotations = getattr(self, "_type_hints", {})
